...
def _turn_input(session_id: uuid.UUID, message: str) -> GraphState:
    return {"session_id": str(session_id), "messages": [HumanMessage(content=message)]}


def _config_for(session_id: uuid.UUID) -> RunnableConfig:
...
async def test_greeting_returns_canned_response_without_retrieval(
    ...
    async with db_session_factory() as session:
        ...
        result = await graph.ainvoke(
            _turn_input(session_id, "hello"), config=_config_for(session_id)
        )
...
async def test_first_turn_missing_profile_triggers_clarification(
    ...
    async with db_session_factory() as session:
        ...
        result = await graph.ainvoke(
            _turn_input(session_id, "How do I apply for OPT?"), config=_config_for(session_id)
        )
...
async def test_ambiguous_question_triggers_topic_clarification(
    ...
    async with db_session_factory() as session:
        ...
        result = await graph.ainvoke(
            _turn_input(session_id, "asdfghjkl qwerty"), config=_config_for(session_id)
        )
...
async def test_full_question_flow_returns_grounded_answer_with_citations(
    ...
    async with db_session_factory() as session:
        ...
        result = await graph.ainvoke(
            _turn_input(session_id, "Where do freshmen live on campus?"),
...
async def test_checkpointer_persists_messages_across_turns(
    ...
    async with db_session_factory() as session:
        ...
        first = await graph.ainvoke(_turn_input(session_id, "hello"), config=config)
        second = await graph.ainvoke(_turn_input(session_id, "hi again"), config=config)
...
async def test_clarification_asked_once_then_question_proceeds(
    ...
    async with db_session_factory() as session:
        ...
        first = await graph.ainvoke(
            _turn_input(session_id, "What meal plans are available?"), config=config
        )
        ...
        second = await graph.ainvoke(
            _turn_input(session_id, "I meant the dining hall meal plans"), config=config
        )
